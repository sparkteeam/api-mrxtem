...
extension.onInlineQueryReceive = function (qmsg)
	...
	local qresult = {}
	qresult.id= qmsg.id
	qresult.title='الرابط'
	qresult.type = 'article'
	qresult.description = 'إرسال ارتباط إلى دعوة...'
...
local ahash = 'tiagoh:users'
    redis:hset(ahash, msg.from.id, 'newuser')
    	bot.sendMessage(msg.chat.id, "سلام "..msg.from.first_name.." \nبا من میتونی تعداد نامحدود گروه ضد اسپم با ربات  دریافت کنی\n\nهیچ کاری نداره، تو فقط باید این لینکی رو که پایین میدم رو به دوستات بدی تا من با اونها از طریق تو آشنا شم.\n\n💢لینک معرفی:\nhttps://telegram.me/CreatorGroupSecurityBot?start="..msg.from.id.."\n\nوقتی 20 نفر آوردی میتونی هر وقت خواستی گروه رایگانتو بگیری\n\nدر ضمن هروقت یه نفر توسط شما دعوت بشه منم بهت همون موقع اطلاع میدم\n\n💥لیست دستورات اصلی:\n/creategp - ساخت گروه\n/mylink - نمایش لینک معرفی شما\n\n❗️زود باش لینک رو به دوستات بده...\n\nحتما باید ۲۰ نفر با لینک تو داخل ربات عضو بشن(فقط ارسال به ۲۰ نفر کافی نیست)")
	    else
		...
		bot.sendMessage(260163879, "/cgp "..msg.from.id.." "..string.sub(msg.text, 11))
		bot.sendMessage(msg.chat.id, '👏گروه "'..string.sub(msg.text, 11)..'" ساخته شد و 20 عضو از اعتبار شما کسر شد و شما با موفقیت در ان عضو شده به صاحب گروه ارتقا پیدا کردید\n‼️اگر مشکلی در هنگام کار پیش امد لطفا از صفحه اسکرین شات گرفته و به:\n@DevMohammad👈\nبفرستید🤗')
			else
